...
const MetamaskConnection = (props) => {
    ...
    const accountDisplay = (account) =>{
      return account.substring(0, 6) + "..." + account.substring(account.length-4, account.length-1);
    }
    ...
    return (
      <button disabled={isDisabled} onClick={onClick}>
        ...
        <span hidden={accounts.length == 0}>
          <BlockieIdenticon address={accounts.length > 0 ? accounts[0] : ""} diameter="20px" alt="BlockieIdenticon"/>
        </span>
      </button>
    );
};
